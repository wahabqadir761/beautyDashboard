...
const EditTeam = () => {
    ...
    let sendData = () => {
        ...
        if (model?._id) {
            ...
            Put("updateteam", model._id, formData, token)
                .then((res) => {
                    console.log("API call successful", res);
                    toast.success('Team updated successfully');
                    navigation("/dashboard/TeamList");
                })
                .catch((error) => {
            ...
            Post("createteam", formData, token).then((res) => {
                navigation("/dashboard/TeamList");
            }).catch((e) => {
                console.log(e)
            })
    ...
    return (
        <div>
            <MainHeader />
            <Header screenTitle={`${model?._id ? "Edit Team" : "Create Team"}`} buttonList={[
                {
                    displayField: () => (
                ...
                <Box >
                    ...
                    <Box className='row p-3'>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'start' }} className='  py-3'>
                            <BsButton text={`${model?._id ? "Edit" : "Create Team"}`} onClick={sendData} />
                        </Box>
                    </Box>
                </Box>
            </Box>
